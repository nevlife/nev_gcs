...
// key-value row. value may contain HTML.
...
// progress bar — auto-colors at 70% (warn) and 90% (error)
function bar(pct) {
...
class CommandCenter {
  ...
  _renderHeader(s) {
    ...
    const eb = $('estop-btn');
    eb.textContent = s.control.estop ? '&#9632; RELEASE' : '&#9632; E-STOP';
  ...
  _renderJoy(ctrl) {
    ...
    const conCls   = ctrl.joystick_connected ? 'green' : 'red';

    $('joy-body').innerHTML =
      kv('status', dot(ctrl.joystick_connected, conCls) +
                   (ctrl.joystick_connected ? 'CONNECTED' : 'DISCONNECTED'), conCls) +
